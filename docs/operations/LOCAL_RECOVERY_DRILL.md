...
- The expected schema head is `20260831124636_phase_2d_private_invitation_coordination.sql`. A different head fails closed until this runbook and the drill are reviewed together.
...
1. Restore the database into the quarantined project and reconcile the authoritative migration head, schema, tenant boundaries, RLS/FORCE RLS, policies, grants, routines, data counts, and content-safe digests.
   Review raw ACL representation and grantor provenance in that new cluster; the local nineteen-object exception is not a portable production assumption.
