...
- The isolated validator uses an allowlisted Auth identity with no family membership rather than a general service key. It leases one exact private job, downloads quarantine into an invocation-scoped bounded spool, incrementally counts and hashes every byte, identifies the format independently of caller metadata, and forces a bounded full decode. It uploads the exact spool bytes without upsert to the job's fresh canonical path, downloads and hashes the canonical object again, and only then may atomically record an immutable original. A late TUS completion can change only quarantine; it cannot alter the captured spool or browser-unwritable canonical object.
