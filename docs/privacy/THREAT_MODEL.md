...
- The isolated validator uses an allowlisted Auth identity with no family membership rather than a general service key. Allowlisting and family membership are mutually exclusive in both directions, revocation locks the same allowlist row as every coordinator, and an expired job can move only to a different validator identity with a new random lease attempt and canonical path. The validator downloads quarantine into an invocation-scoped bounded spool, incrementally counts and hashes every byte, identifies the format independently of caller metadata, proves the exact codestream boundary, and forces a bounded full decode. It uploads the exact fully consumed spool stream without upsert to the attempt-specific canonical path, downloads and hashes the canonical object again, and only then may atomically record an immutable original. A late TUS completion can change only quarantine; it cannot alter the captured spool or browser-unwritable canonical object. A stale validator loses source and canonical authority when a different identity reclaims the lease.
- The Phase 4B local boundary accepts only JPEG, PNG, and WebP validation. HEIC/HEIF remains quarantined and unpublished until the exact isolated Linux decoder artifact passes synthetic and genuine iPhone fixtures under measured resource limits. Validation creates no moment, display derivative, browser media read, signed URL, export entry, or deletion promise.
