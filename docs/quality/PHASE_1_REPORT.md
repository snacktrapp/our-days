...
| Local browser matrix                                          | `npm run test:e2e`: atomic webpack build and private-artifact scan passed; 113 browser checks passed, 47 intentional project-specific skips, 0 failures                                                                                                                                                                             |
...
- The first isolated GitHub run must execute the prepared functional browser projects and calibrate the reviewed local snapshots against the x64 macOS 15 host. The complete CI list contains 210 checks across 9 files, including WebKit and project-specific visual cases. Baseline differences must be inspected and approved; CI never updates them automatically.
