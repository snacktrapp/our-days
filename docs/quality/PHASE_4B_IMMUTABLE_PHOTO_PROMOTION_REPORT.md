...
- A canonical Storage request authorized just before revocation may still finish. Its attempt-specific object remains an unpublished orphan: terminal completion re-locks the allowlist row, observes revocation, and refuses to create an original. Cleanup timing remains governed by the pending PD-005 decision.
...
- Twenty-one focused byte-validator contracts cover size/hash/MIME mismatch, complete decode limits, exact JPEG/PNG/WebP endings, concatenated/trailing JPEG rejection, stalled input deadlines, random chunking, private-spool cleanup, full handoff consumption, and escaped-stream denial. The complete Vitest run passes 519 tests across 45 files.
- The full pgTAP run passes 764 assertions across 14 files. It pins table/routine/trigger catalogs, exact ACLs and policies, family/validator identity separation, lease ownership and distinct-identity reclamation, revocation behavior, nullable argument denial, requester-authority loss, immutable original consistency, and zero-publication outcomes. Supabase schema lint reports no errors, generated public TypeScript types match, and two consecutive clean resets pass.
- The real local Storage journey proves exact quarantine read, bounded validation, exact-byte no-upsert canonical upload, canonical read-back, atomic completion, duplicate denial, late-quarantine-write isolation, and post-completion browser/validator denial. A second real journey pre-seeds exact-size wrong canonical bytes through the local service fixture boundary, proves no-upsert duplicate denial and `PHOTO_HASH_MISMATCH`, and terminalizes as `canonical_collision` with no original or photo moment.
- The concurrency journey preserves Phase 4A's distinct-TUS-URL overwrite finding and proves that a completion visibly waits behind validator revocation, is denied after revocation commits, and leaves the intake, job, original, and audit ledgers unchanged. The broader database concurrency and Auth/OTP/invitation/revocation integrations also pass in isolation.
- The logical restore drill passes from committed source through exact schema, catalog, normalized-data, archive, restored-schema, ACL, authorization, migration-history, and cleanup checks. It continues to state that database recovery does not restore Storage bytes or non-empty Auth sessions.
- The production webpack build and private-artifact scan pass. The complete local mobile/visual Playwright suite passes 161 applicable tests with 55 intentional project/viewport skips. The full connected family journey passes in Chromium and Firefox. Firefox exposed a rejected service-worker icon interception during this review; narrowing interception to navigation and the offline stylesheet fixed it, and the exact failing journey plus installed-PWA offline/cache tests now pass. Local WebKit remains unavailable on this macOS 14 Intel host, so the macOS 15 Intel CI WebKit job remains authoritative.
