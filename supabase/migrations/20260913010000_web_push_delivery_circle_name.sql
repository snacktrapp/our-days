-- Push banners must name the circle the tap opens.
-- Replay-safe: drop + recreate so RETURNS TABLE can add circle_name.
...
begin
  ...
  if requested_activity_kind = 'moment' then
    ...
    select distinct on (subscription.endpoint)
      ...
      join public.circles as visible
        on visible.id = chosen.circle_id
  ...
  if requested_activity_kind = 'note' then
    ...
    if resolved_moment_id is null
      ...
      or actor_user_id = owner_user_id then
      return;
    end if;
    ...
    select distinct on (subscription.endpoint)
      ...
      join public.circles as visible
        on visible.id = target_circle_id
