-- Replace repeated pg_timezone_names scans in write RPCs with a seeded table.
-- Keeps the accepted timezone name set aligned with pg_timezone_names.

create table if not exists private.valid_time_zones (
  name text primary key
);

delete from private.valid_time_zones as existing
where not exists (
  select 1
    from pg_catalog.pg_timezone_names as zone
   where zone.name = existing.name
);

insert into private.valid_time_zones (name)
select distinct zone.name
  from pg_catalog.pg_timezone_names as zone
 where zone.name is not null
on conflict (name) do nothing;

create or replace function private.is_valid_time_zone(requested_time_zone text)
returns boolean
language sql
stable
set search_path = ''
as $$
  select exists (
    select 1
      from private.valid_time_zones as zone
     where zone.name = requested_time_zone
  );
$$;
