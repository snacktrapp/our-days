-- Operations may upload and attach a poster on a Just me Insight it created
-- for a consenting member. The read gate stays local to the poster path and
-- attach functions so timeline, feed, delivery, notes, and reactions stay closed.

create function private.operations_can_manage_member_insight(moment_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1
      ...
      join public.circle_memberships as actor
        on actor.circle_id = moment.circle_id
       and actor.id = moment.created_by_operations_membership_id
       and actor.user_id = (select auth.uid())
       and actor.status = 'active'
       and actor.role = 'organizer'
       and actor.directory_kind = 'operations'
      join public.circle_memberships as recorder
       ...
       and recorder.directory_kind = 'journal'
       and recorder.user_id is not null
      join private.operations_just_me_insight_consent as consent
        on consent.user_id = recorder.user_id
     where moment.id = moment_id
       ...
       and (select private.is_circle_organizer(moment.circle_id))
  );
$$;

revoke all on function private.operations_can_manage_member_insight(uuid)
  from public, anon, authenticated;
grant execute on function private.operations_can_manage_member_insight(uuid)
  to authenticated;
  ...
  return exists (
    select 1
     ...
     where video.moment_id = target_moment_id
       ...
       and (select private.current_family_session_is_live())
       and (
         (select private.can_read_live_moment(moment.id))
         or (select private.operations_can_manage_member_insight(moment.id))
       )
...
begin
  ...
  if actor_membership_id is null
    or not (
      (select private.can_read_live_moment(target_video.moment_id))
      or (select private.operations_can_manage_member_insight(target_video.moment_id))
    ) then
    raise exception using errcode = '42501',
      message = 'Video poster could not be saved';
  end if;
