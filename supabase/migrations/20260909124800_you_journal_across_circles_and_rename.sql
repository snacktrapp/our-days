-- YOU / Just me lists moments on the signed-in user's person rows across
-- every active membership. Circle Home feeds stay circle-scoped. Circle
-- rename is starter-only and does not grant table UPDATE.

create function private.update_circle(
...
begin
  if actor_user_id is null
    ...
    raise exception using
      errcode = '22023',
      message = 'Circle could not be renamed';
  end if;

  select membership.user_id, membership.status
    into starter_user_id, starter_status
    from public.circles as circle
    join public.circle_memberships as membership
      on membership.circle_id = circle.id
     and membership.id = circle.created_by_membership_id
   where circle.id = requested_circle_id
   for update of circle;

  if starter_user_id is null
    or starter_user_id is distinct from actor_user_id
    or starter_status is distinct from 'active'
  ...
  update public.circles
   ...
   where id = requested_circle_id;

  return requested_circle_id;
end;
$$;
...
set search_path = ''
as $$
  select private.update_circle(circle_id, circle_name);
$$;

revoke all on function private.update_circle(uuid, text)
  from public, anon, authenticated;
revoke all on function public.update_circle(uuid, text)
  from public, anon, authenticated;
grant execute on function private.update_circle(uuid, text) to authenticated;
